// ═══════════════════════════════════════════
//  ASCENDED HERO: "Swampborne Waflav"
//  400 HP, 180 ATK — Waflav archetype
//
...
//   turn. Whenever a target is Poisoned, place 1 Evolution
//   Counter onto this Hero. You may once per turn Descend
//   this Hero to place 1 Evolution Counter onto it."
//
//  "Whenever a target is Poisoned" — Als Ruling: EVERY target
...
const CARD_NAME = 'Swampborne Waflav';
const ASCEND_COST = 2;
const DESCEND_GAIN = 1;
const TRIGGER_STATUS = 'poisoned';
...
module.exports = {
  ...
  /**
   * On Ascension: "Choose any target on the board and apply 1 Stack of Poison to it."
  ...
  async onAscensionBonus(engine, pi, heroIdx) {
    await W.applyStatusOnAscension(engine, pi, CARD_NAME, 'poisoned', {
      stacks: 1, animationType: 'poison_tick',
      description: 'Choose any target on the board and apply 1 Stack of Poison to it.',
  ...
  hooks: {
    ...
    /** "Whenever a target is Poisoned, place 1 Evolution Counter onto this Hero." */
  ...
  cpuMeta: {
    ...
    // Zaehler, sobald IRGENDEIN Ziel vergiftet wird — richtig, wenn ein
    // Poison-Applier zur Hand ist (Als Ruling 6.8.).
    // Deckneutraler Vertrag: die Zugende-Messung liest ihn, statt
    // Formnamen zu kennen.
    counterSource: { kind: 'status', status: 'poisoned' },
