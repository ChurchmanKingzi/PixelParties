...
const CARD_NAME = 'Minocrete War Counsellor';
...
function fromOtherCounsellor(engine, source, pi) {
  ...
  return isWarCounsellorCreatureName(engine, source.name);
}
...
module.exports = {
  ...
  hooks: {
    ...
    beforeDamage: (ctx) => {
      ...
      if (!fromOtherCounsellor(ctx._engine, ctx.source, ctx.cardOwner)) return;
    ...
    beforeCreatureDamageBatch: (ctx) => {
      ...
      for (const e of (ctx.entries || [])) {
        ...
        if (!fromOtherCounsellor(ctx._engine, e.source, ctx.cardOwner)) continue;
